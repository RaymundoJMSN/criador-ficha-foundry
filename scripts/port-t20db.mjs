...
// 8. racas.json — all raças consolidated
{
  ...
  // O T20-DB marca "atributos_diferentes" em raças cujo texto NÃO tem a palavra.
  // "+1 em dois atributos" sem "diferentes" deixa pôr os dois no mesmo atributo
  // (Mashin e Minauro em Dragão Brasil, Kallyanach em Ameaças p.151, Meio-Elfo
  // em HA p.45: "Int +1, +1 em dois atributos, exceto Constituição").
  const SEM_DIFERENTES = new Set(["mashin", "minauro", "kallyanach", "meio_elfo"]);
  function escolhasDe(lista, racaId = "") {
    return (lista ?? []).map((e) => ({
      valor: e.valor,
      quantidade: e.quantidade,
      atributos_diferentes: SEM_DIFERENTES.has(racaId) ? false : (e.atributos_diferentes ?? false),
  ...
  for (const f of readdirSync(racasDir)) {
    ...
    if (mod.tipo === "alternativo") {
      const alts = mod.alternativas ?? [];
      // Suraggel: cada alternativa tem fixos próprios e é um item de raça no
      // compêndio (Aggelus, Sulfure). Kallyanach: as alternativas são só dois
      // MODOS de distribuir atributo ("+2 em um ou +1 em dois") da MESMA raça —
      // virar duas raças "alt_um_atributo"/"alt_dois_atributos" escondia a
      // Herança Dracônica, presa a ids que item nenhum tem. Duende e Moreau vêm
      // sem alternativas (construtor por passos / 12 heranças no compêndio) e
      // entram só com o comum.
      const soModos = alts.length > 0 && alts.every((a) => !a.fixos?.length && a.escolhas?.length);
      if (alts.length > 0 && !soModos) {
        for (const alt of alts) {
          racas.push({
            ...
            atributos_escolha: escolhasDe(alt.escolhas, r.id),
          });
        }
        continue;
      }
      const modos = alts
        .map((a) => escolhasDe(a.escolhas, r.id)[0])
        .filter(Boolean)
      ...
      racas.push({
        ...
        ...comum,
        atributos_fixos: [],
        atributos_escolha: principal
          ? [{ ...principal, ...(modos[1] ? { alternativa: { valor: modos[1].valor, quantidade: modos[1].quantidade } } : {}) }]
          : [],
      });
      continue;
    }
    ...
    racas.push({
      ...
      atributos_escolha:
        mod.tipo === "escolha" || mod.tipo === "misto" ? escolhasDe(mod.escolhas, r.id) : [],
    });
  }
