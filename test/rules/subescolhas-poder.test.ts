...
describe("sub-escolhas de poder", () => {
  it("regra estruturada pelo slug do item; texto com 'escolha um' vira campo opcional", () => {
    ...
    expect(generica).toMatchObject({ tipo: "texto", opcional: true });
    expect(subEscolhaDoPoder("Ataque Especial", "Você pode gastar 1 PM para receber +4.")).toBeNull();
  });

  it("opcional não vira pendência; estruturada vira", () => {
    const adq = [
    ...
    expect(respostasDeSubEscolhas(adq, { sp_aspirante_a_heroi_0: "for", sp_furia_elemental_0: "fogo" }).map((r) => r.valor)).toEqual(["for", "fogo"]);
  });
});
